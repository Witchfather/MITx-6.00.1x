...
def buildCoder(shift):
    ...
    for idx, c in enumerate(string.ascii_lowercase):
        coder[c] = string.ascii_lowercase[(idx + shift) % 26]

    for idx, c in enumerate(string.ascii_uppercase):
        coder[c] = string.ascii_uppercase[(idx + shift) % 26]
...
def findBestShift(wordList, text):
    ...
    for guess in range(0, 26):
        validWords = 0
        words = applyShift(text, (25 - guess)).split()
        for word in words:
            if isWord(wordList, word):
                validWords += 1
        if validWords > maxValidWords:
            maxValidWords, bestShift = validWords, (25 - guess)
    return bestShift
